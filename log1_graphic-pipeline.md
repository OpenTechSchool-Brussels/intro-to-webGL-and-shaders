...
After these vertex transformations, the renderer will form triangles out of the list of vertex position. This step can be configured to alternatively form lines or point. We will stick here to the triangle case. 
...
The programmable fragment shader will run for every fragment defined in the rasterization process, allowing to chose what will be the final color on the fragment. It can depend on textures, on light calculation, etc ...

The final step is to discard fragments that don't pass visibility tests. These tests can include for example depht test : if one triangle is obscured by another triangle, the fragment of the closer triangle should end up on the screen. We get finally the final pixel color !
